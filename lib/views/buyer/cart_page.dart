...
class _CartPageState extends State<CartPage> {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: cart.items.isEmpty
            ...
            : Column(
                children: [
                  _buildHeader(cart, allSelected),
                  _buildCartList(cart),
                  _buildFooter(cart, selectedCount, selectedTotal),
                ],
              ),
      ),
    );
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const Text('Keranjang Kosong', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
          const SizedBox(height: 12),
          Text('Tambahkan bunga cantik ke keranjang Anda', style: TextStyle(fontSize: 14, color: Colors.grey.shade600)),
          const SizedBox(height: 30),
          ElevatedButton(
            onPressed: () {
              // Navigate to home
  ...
  Widget _buildHeader(CartProvider cart, bool allSelected) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          Row(
            children: [
              const Text('Keranjang', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
              const SizedBox(width: 12),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                decoration: BoxDecoration(color: AppColors.accentPink, borderRadius: BorderRadius.circular(20)),
                child: Text('${cart.getTotalItems()} item', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: AppColors.primary)),
              ),
            ],
          ),
          GestureDetector(
  ...
  Widget _buildFooter(CartProvider cart, int selectedCount, int selectedTotal) {
    return Container(
      ...
      child: Column(
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text('Dipilih ($selectedCount)', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: selectedCount == 0 ? null : () {
                final selectedItemsList = cart.items.where((item) => selectedItems[item.bouquet.id] == true).toList();
                final total = selectedItemsList.fold<int>(0, (sum, item) => sum + (item.price * item.quantity));

                _showOrderConfirmationDialog(selectedItemsList, total, cart);
              },
              style: ElevatedButton.styleFrom(
